      ...
      <ul>
        {thumbs.map(thumb => (
          <li key={thumb.name}>
            ...
            <button onClick={() => getCropHint(thumb.name, image.id, [thumb.width, thumb.height])}>Preview Smart Crop</button>
            <button onClick={saveCrop}>Crop & <span>Overwrite</span></button>
          </li>
        ))}
      </ul>
    </div>
